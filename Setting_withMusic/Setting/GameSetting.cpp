...
/*
Memo: 
Turn on the background music -> Press "B"
Turn off the background music -> Press "E"
*/
#include <stdio.h>
#include <stdlib.h>//
#include <math.h>
...
void run(int width, int height, int mapWidth, int mapHeight) {
	...
	bool timeoutNoticed = false;

	// load .wav files
	YsSoundPlayer::SoundData propeller, jumpNotice, timeoutNotice;
	if (YSOK != propeller.LoadWav("melody.wav"))
	{
		printf("Error!  Cannot load melody.wav!\n");
	}
	if (YSOK != jumpNotice.LoadWav("smb_jump-small.wav"))
	{
		printf("Error!  Cannot load smb_jump-small.wav!\n");
	}
	if (YSOK != timeoutNotice.LoadWav("smb_warning.wav"))
	{
		printf("Error!  Cannot load smb_warning.wav!\n");
	}
	//////////////////////////////////////////////////////////
	...
	while (FsInkey() != FSKEY_ESC)
	{
		...
		if (myGame.timer > 100 && !timeoutNoticed) {
			player.Stop(timeoutNotice);
			player.PlayOneShot(timeoutNotice);
			timeoutNoticed = true;
		}
		//keyPressed for sound
		if (FSKEY_SPACE == key)
		{
