...
YSRESULT YsSoundPlayer::SoundData::LoadWav(const char fn[])
{
	...
	if(fp!=NULL)
	{
		// I have seen some horrible stdio.h not having SEEK_SET and SEEK_END.
		...
		auto res=LoadWavFromMemory(fsize,buf);
		if(YSOK!=res)
		{
			printf("Err!\n");
		}
...
int YsSoundPlayer::SoundData::GetSignedValue(int atTimeStep,int channel) const
{
...
void YsSoundPlayer::SoundData::SetSignedValue(unsigned char *savePtr,int rawSignedValue)
{
	switch(bit)
	{
	case 8:
